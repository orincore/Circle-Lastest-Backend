-- Update find_nearby_users function to filter by verification_required instead of verification_status
-- Only show users who don't require face verification and have email verification
...
BEGIN
  ...
  WHERE 
    ...
    AND p.verification_required = FALSE
...
-- Add comment
COMMENT ON FUNCTION find_nearby_users IS 'Finds nearby users within specified radius. Only returns users who do not require face verification and have completed email verification.';
