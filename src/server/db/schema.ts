import { pgTable, index, foreignKey, pgPolicy, bigserial, varchar, uuid, timestamp, jsonb, check, text, numeric, boolean, unique, integer, uniqueIndex, date, time, serial, bigint, doublePrecision, inet, vector, primaryKey, pgView, pgMaterializedView, pgEnum, pgSchema } from "drizzle-orm/pg-core"
...
export const watchPartyStatus = pgEnum("watch_party_status", ['active', 'ended'])
...
export const analyticsEvents = pgTable("analytics_events", {
	...
	pgPolicy("Service role can manage all analytics events", { as: "permissive", for: "all", to: ["public"], using: sql`(auth.role() = 'service_role'::text)` }),
	pgPolicy("Users can view their own analytics events", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const appVersions = pgTable("app_versions", {
	...
	pgPolicy("Service role can manage all app versions", { as: "permissive", for: "all", to: ["public"], using: sql`(auth.role() = 'service_role'::text)` }),
	pgPolicy("Users can view their own app versions", { as: "permissive", for: "select", to: ["public"] }),
...
export const betaTesters = pgTable("beta_testers", {
	...
	pgPolicy("beta_testers_insert_policy", { as: "permissive", for: "insert", to: ["public"], withCheck: sql`true`  }),
	pgPolicy("beta_testers_select_policy", { as: "permissive", for: "select", to: ["public"] }),
	pgPolicy("beta_testers_update_policy", { as: "permissive", for: "update", to: ["public"] }),
...
export const giverRequestAttempts = pgTable("giver_request_attempts", {
	...
	pgPolicy("Givers can update their own attempts", { as: "permissive", for: "update", to: ["public"], using: sql`(auth.uid() = giver_user_id)` }),
	...
	pgPolicy("giver_attempts_view_own", { as: "permissive", for: "select", to: ["public"] }),
...
export const profiles = pgTable("profiles", {
	...
	pgPolicy("Users can update own location", { as: "permissive", for: "update", to: ["public"], using: sql`(auth.uid() = id)` }),
	pgPolicy("Users can view others location", { as: "permissive", for: "select", to: ["public"] }),
...
export const appVersionConfig = pgTable("app_version_config", {
	...
	pgPolicy("Allow public read access to app version config", { as: "permissive", for: "select", to: ["public"], using: sql`true` }),
...
export const aiConversations = pgTable("ai_conversations", {
	...
	pgPolicy("ai_conversations_anonymous_policy", { as: "permissive", for: "all", to: ["public"], using: sql`((user_id IS NULL) OR (auth.uid() = user_id) OR (auth.uid() IN ( SELECT admin_roles.user_id
   FROM admin_roles
  WHERE (admin_roles.is_active = true))))` }),
	pgPolicy("ai_conversations_user_policy", { as: "permissive", for: "all", to: ["public"] }),
...
export const blocks = pgTable("blocks", {
	...
	pgPolicy("Users can create blocks", { as: "permissive", for: "insert", to: ["public"], withCheck: sql`((auth.uid())::text = (blocker_id)::text)`  }),
	pgPolicy("Users can delete their own blocks", { as: "permissive", for: "delete", to: ["public"] }),
	pgPolicy("Users can view their own blocks", { as: "permissive", for: "select", to: ["public"] }),
...
export const chatUserSettings = pgTable("chat_user_settings", {
	...
	pgPolicy("chat_user_settings_delete", { as: "permissive", for: "delete", to: ["public"], using: sql`(user_id = auth.uid())` }),
	...
	pgPolicy("chat_user_settings_update", { as: "permissive", for: "update", to: ["public"] }),
]);
...
export const chatMuteSettings = pgTable("chat_mute_settings", {
	...
	pgPolicy("Users can delete own mute settings", { as: "permissive", for: "delete", to: ["public"], using: sql`(auth.uid() = user_id)` }),
	...
	pgPolicy("Users can view own mute settings", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const exploreInteractions = pgTable("explore_interactions", {
	...
	pgPolicy("explore_interactions_delete_own", { as: "permissive", for: "delete", to: ["public"], using: sql`(auth.uid() = user_id)` }),
	pgPolicy("explore_interactions_insert_own", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("explore_interactions_select_own", { as: "permissive", for: "select", to: ["public"] }),
...
export const featureUsage = pgTable("feature_usage", {
	...
	pgPolicy("Service role can manage all feature usage", { as: "permissive", for: "all", to: ["public"], using: sql`(auth.role() = 'service_role'::text)` }),
	pgPolicy("Users can view their own feature usage", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const helpSessionFeedback = pgTable("help_session_feedback", {
	...
	pgPolicy("Participants can insert feedback", { as: "permissive", for: "insert", to: ["public"], withCheck: sql`((auth.uid() = receiver_user_id) OR (auth.uid() = giver_user_id))`  }),
	pgPolicy("Participants can update their own feedback", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Participants can view feedback for their sessions", { as: "permissive", for: "select", to: ["public"] }),
...
export const memeComments = pgTable("meme_comments", {
	...
	check("meme_comments_status_check", sql`(status)::text = ANY (ARRAY[('active'::character varying)::text, ('hidden'::character varying)::text, ('flagged'::character varying)::text, ('deleted'::character varying)::text])`),
]);
...
export const memeConnectRequests = pgTable("meme_connect_requests", {
	...
	index("idx_meme_connect_requester").using("btree", table.requesterId.asc().nullsLast().op("text_ops"), table.status.asc().nullsLast().op("uuid_ops")),
	index("idx_meme_connect_requests_chat_id").using("btree", table.chatId.asc().nullsLast().op("uuid_ops")),
	index("idx_meme_connect_target").using("btree", table.targetId.asc().nullsLast().op("text_ops"), table.status.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.chatId],
			foreignColumns: [chats.id],
	...
	check("meme_connect_requests_status_check", sql`(status)::text = ANY (ARRAY[('pending'::character varying)::text, ('accepted'::character varying)::text, ('declined'::character varying)::text, ('expired'::character varying)::text])`),
]);
...
export const messageReactions = pgTable("message_reactions", {
	...
	pgPolicy("Users can add message reactions", { as: "permissive", for: "insert", to: ["public"], withCheck: sql`((user_id = auth.uid()) AND (EXISTS ( SELECT 1
   FROM (messages m
  ...
  WHERE ((m.id = message_reactions.message_id) AND (cm.user_id = auth.uid())))))`  }),
	...
	pgPolicy("Users can view message reactions", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const memeSources = pgTable("meme_sources", {
	...
	check("meme_sources_status_check", sql`(status)::text = ANY (ARRAY[('active'::character varying)::text, ('backoff'::character varying)::text, ('disabled'::character varying)::text])`),
]);
...
export const messageReceipts = pgTable("message_receipts", {
	...
	pgPolicy("Users can create message receipts", { as: "permissive", for: "insert", to: ["public"], withCheck: sql`((user_id = auth.uid()) AND (message_id IN ( SELECT messages.id
   FROM messages
  WHERE (messages.sender_id <> auth.uid()))))`  }),
	pgPolicy("Users can update their own receipts", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can view message receipts", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const paymentTransactions = pgTable("payment_transactions", {
	...
	check("payment_transactions_source_check", sql`(source)::text = ANY (ARRAY[('ios'::character varying)::text, ('android'::character varying)::text, ('web'::character varying)::text])`),
	check("payment_transactions_status_check", sql`(status)::text = ANY (ARRAY[('created'::character varying)::text, ('success'::character varying)::text, ('failed'::character varying)::text, ('refunded'::character varying)::text])`),
]);
...
export const userConsent = pgTable("user_consent", {
	...
	pgPolicy("Service role can view all consent records", { as: "permissive", for: "select", to: ["public"], using: sql`(auth.role() = 'service_role'::text)` }),
	pgPolicy("Users can manage their own consent", { as: "permissive", for: "all", to: ["public"] }),
]);
...
export const subscriptionPlans = pgTable("subscription_plans", {
	...
	check("subscription_plans_billing_period_check", sql`(billing_period)::text = ANY (ARRAY[('monthly'::character varying)::text, ('yearly'::character varying)::text])`),
]);
...
export const userSubscriptions = pgTable("user_subscriptions", {
	...
	check("user_subscriptions_source_check", sql`(source)::text = ANY (ARRAY[('ios'::character varying)::text, ('android'::character varying)::text, ('web'::character varying)::text])`),
	check("user_subscriptions_status_check", sql`(status)::text = ANY (ARRAY[('active'::character varying)::text, ('cancelled'::character varying)::text, ('expired'::character varying)::text, ('grace_period'::character varying)::text, ('pending'::character varying)::text])`),
]);
...
export const userSessions = pgTable("user_sessions", {
	...
	pgPolicy("Service role can manage all sessions", { as: "permissive", for: "all", to: ["public"], using: sql`(auth.role() = 'service_role'::text)` }),
	pgPolicy("Users can view their own sessions", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const memeAssets = pgTable("meme_assets", {
	...
	check("meme_assets_asset_type_check", sql`(asset_type)::text = ANY (ARRAY[('image'::character varying)::text, ('video'::character varying)::text, ('thumbnail'::character varying)::text])`),
]);
...
export const emailOtps = pgTable("email_otps", {
	...
	pgPolicy("email_otps_service_policy", { as: "permissive", for: "all", to: ["public"], using: sql`(auth.role() = 'service_role'::text)` }),
	pgPolicy("email_otps_user_policy", { as: "permissive", for: "all", to: ["public"] }),
]);
...
export const crashReports = pgTable("crash_reports", {
	...
	pgPolicy("Service role can manage all crash reports", { as: "permissive", for: "all", to: ["public"], using: sql`(auth.role() = 'service_role'::text)` }),
	pgPolicy("Users can view their own crash reports", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const giverProfiles = pgTable("giver_profiles", {
	...
	pgPolicy("Users can delete their own giver profile", { as: "permissive", for: "delete", to: ["public"], using: sql`(auth.uid() = user_id)` }),
	...
	pgPolicy("Users can view all giver profiles", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const userPhotos = pgTable("user_photos", {
	...
	pgPolicy("Users can delete their own photos", { as: "permissive", for: "delete", to: ["public"], using: sql`(user_id = auth.uid())` }),
	...
	pgPolicy("Users can view their own photos", { as: "permissive", for: "select", to: ["public"] }),
...
export const adminAuditLogs = pgTable("admin_audit_logs", {
	...
	pgPolicy("admin_audit_logs_insert_policy", { as: "permissive", for: "insert", to: ["public"], withCheck: sql`true`  }),
	pgPolicy("admin_audit_logs_select_policy", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const helpRequests = pgTable("help_requests", {
	...
	pgPolicy("Users can create their own help requests", { as: "permissive", for: "insert", to: ["public"], withCheck: sql`(auth.uid() = receiver_user_id)`  }),
	pgPolicy("Users can update their own help requests", { as: "permissive", for: "update", to: ["public"] }),
	pgPolicy("Users can view their own help requests", { as: "permissive", for: "select", to: ["public"] }),
...
export const pushTokens = pgTable("push_tokens", {
	...
	uniqueIndex("idx_push_tokens_user_device").using("btree", table.userId.asc().nullsLast().op("uuid_ops"), table.deviceId.asc().nullsLast().op("uuid_ops")).where(sql`(device_id IS NOT NULL)`),
	index("idx_push_tokens_user_id").using("btree", table.userId.asc().nullsLast().op("uuid_ops")),
	uniqueIndex("idx_push_tokens_user_token").using("btree", table.userId.asc().nullsLast().op("text_ops"), table.token.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [profiles.id],
...
export const userReports = pgTable("user_reports", {
	...
	pgPolicy("user_reports_insert_policy", { as: "permissive", for: "insert", to: ["public"], withCheck: sql`(auth.uid() = reporter_id)`  }),
	pgPolicy("user_reports_select_policy", { as: "permissive", for: "select", to: ["public"] }),
	pgPolicy("user_reports_update_policy", { as: "permissive", for: "update", to: ["public"] }),
...
export const userActivityEvents = pgTable("user_activity_events", {
	...
	pgPolicy("user_activity_events_admin_policy", { as: "permissive", for: "all", to: ["public"], using: sql`(EXISTS ( SELECT 1
   FROM admin_roles
  WHERE (admin_roles.user_id = auth.uid())))` }),
	pgPolicy("user_activity_events_insert_policy", { as: "permissive", for: "insert", to: ["public"] }),
	pgPolicy("user_activity_events_select_policy", { as: "permissive", for: "select", to: ["public"] }),
]);
...
export const engagementNotifications = pgTable("engagement_notifications", {
	...
	index("idx_engagement_notifications_recipient_type").using("btree", table.recipientId.asc().nullsLast().op("text_ops"), table.notificationType.asc().nullsLast().op("text_ops"), table.sentAt.asc().nullsLast().op("text_ops")),
	...
	unique("engagement_notifications_recipient_id_notification_type_ded_key").on(table.recipientId, table.notificationType, table.dedupeKey),
	check("engagement_notifications_notification_type_check", sql`(notification_type)::text = ANY ((ARRAY['meme_liked_by_friend'::character varying, 'meme_discovery'::character varying, 'birthday_self'::character varying, 'friend_birthday'::character varying, 'weather_checkin'::character varying])::text[])`),
]);
...
export const authSessions = pgTable("auth_sessions", {
	...
	unique("auth_sessions_jti_key").on(table.jti),
]);
...
export const jamSessions = pgTable("jam_sessions", {
	...
	uniqueIndex("jam_sessions_one_active_per_chat").using("btree", table.chatId.asc().nullsLast().op("uuid_ops")).where(sql`(status <> 'ended'::jam_session_status)`),
	// No inline foreignKey() here for current_queue_item_id -> jam_session_queue.id
	// (Postgres constraint fk_jam_sessions_current_queue_item still exists and is
	// still enforced -- this only affects Drizzle's own TS-level model of it).
	// jamSessions and jamSessionQueue FK each other (this column one way, plus
	// jam_session_queue.session_id -> jam_sessions.id below) -- declaring *both*
	// directions inline in the same module makes TS unable to resolve either
	// table's type (circular initializer inference), and no cast/thunk broke it
	// cleanly. Dropping just this direction leaves jamSessionQueue's FK back to
	// jamSessions as a normal one-way backward reference, which resolves fine.
	// relations.ts's `jamSessions.currentQueueItem` relation is unaffected --
	// drizzle's `relations()` helper works off column references directly, not
	// off this foreignKey() builder.
	foreignKey({
			columns: [table.chatId],
			foreignColumns: [chats.id],
...
export const jamSessionQueue = pgTable("jam_session_queue", {
	...
	index("idx_jam_queue_session_position").using("btree", table.sessionId.asc().nullsLast().op("float8_ops"), table.position.asc().nullsLast().op("float8_ops")),
	foreignKey({
...
export const jamPlaylistTracks = pgTable("jam_playlist_tracks", {
	...
	index("idx_jam_playlist_tracks_playlist_position").using("btree", table.playlistId.asc().nullsLast().op("float8_ops"), table.position.asc().nullsLast().op("float8_ops")),
	foreignKey({
			columns: [table.addedBy],
			foreignColumns: [profiles.id],
...
export const memes = pgTable("memes", {
	...
	origin: varchar({ length: 20 }).default('scraped').notNull(),
	musicYoutubeVideoId: text("music_youtube_video_id"),
	musicTitle: text("music_title"),
	musicChannelTitle: text("music_channel_title"),
	musicDurationSeconds: integer("music_duration_seconds"),
	musicStartSeconds: integer("music_start_seconds").default(0).notNull(),
	musicTrimSeconds: integer("music_trim_seconds").default(15).notNull(),
}, (table) => [
	...
	index("idx_memes_uploader").using("btree", table.uploaderUserId.asc().nullsLast().op("uuid_ops")).where(sql`(uploader_user_id IS NOT NULL)`),
	foreignKey({
	...
	foreignKey({
			...
			foreignColumns: [profiles.id],
			name: "memes_uploader_user_id_fkey"
		}).onDelete("cascade"),
	unique("memes_instagram_shortcode_key").on(table.instagramShortcode),
	check("memes_origin_check", sql`(origin)::text = ANY ((ARRAY['scraped'::character varying, 'user_upload'::character varying])::text[])`),
	check("memes_origin_owner_check", sql`(((origin)::text = 'scraped'::text) AND (source_id IS NOT NULL)) OR (((origin)::text = 'user_upload'::text) AND (uploader_user_id IS NOT NULL))`),
	check("memes_post_type_check", sql`(post_type)::text = ANY (ARRAY[('image'::character varying)::text, ('carousel'::character varying)::text, ('video'::character varying)::text])`),
	check("memes_status_check", sql`(status)::text = ANY (ARRAY[('active'::character varying)::text, ('hidden'::character varying)::text, ('flagged'::character varying)::text])`),
]);

export const memeGenres = pgTable("meme_genres", {
	memeId: uuid("meme_id").notNull(),
	genre: varchar({ length: 30 }).notNull(),
}, (table) => [
	index("idx_meme_genres_genre").using("btree", table.genre.asc().nullsLast().op("text_ops")),
	foreignKey({
			columns: [table.memeId],
			foreignColumns: [memes.id],
			name: "meme_genres_meme_id_fkey"
		}).onDelete("cascade"),
	primaryKey({ columns: [table.memeId, table.genre], name: "meme_genres_pkey"}),
	check("meme_genres_genre_check", sql`(genre)::text = ANY ((ARRAY['comedy'::character varying, 'relatable'::character varying, 'wholesome'::character varying, 'dark_humor'::character varying, 'animals'::character varying, 'gaming'::character varying, 'anime'::character varying, 'sports'::character varying, 'desi'::character varying, 'tech'::character varying, 'politics'::character varying, 'random'::character varying])::text[])`),
]);
...
export const userGenreAffinity = pgTable("user_genre_affinity", {
	userId: uuid("user_id").notNull(),
	genre: varchar({ length: 30 }).notNull(),
	affinityScore: doublePrecision("affinity_score").default(0).notNull(),
	updatedAt: timestamp("updated_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
}, (table) => [
	index("idx_user_genre_affinity_user_id").using("btree", table.userId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [profiles.id],
			name: "user_genre_affinity_user_id_fkey"
		}).onDelete("cascade"),
	primaryKey({ columns: [table.userId, table.genre], name: "user_genre_affinity_pkey"}),
]);
...
export const watchPartySessions = pgTable("watch_party_sessions", {
	id: uuid().defaultRandom().primaryKey().notNull(),
	hostId: uuid("host_id").notNull(),
	status: watchPartyStatus().default('active').notNull(),
	memeIds: uuid("meme_ids").array().default([]).notNull(),
	currentIndex: integer("current_index").default(0).notNull(),
	createdAt: timestamp("created_at", { withTimezone: true, mode: 'string' }).defaultNow().notNull(),
	endedAt: timestamp("ended_at", { withTimezone: true, mode: 'string' }),
}, (table) => [
	uniqueIndex("watch_party_one_active_per_host").using("btree", table.hostId.asc().nullsLast().op("uuid_ops")).where(sql`(status <> 'ended'::watch_party_status)`),
	foreignKey({
			columns: [table.hostId],
			foreignColumns: [profiles.id],
			name: "watch_party_sessions_host_id_fkey"
		}),
]);
...
export const watchPartyParticipants = pgTable("watch_party_participants", {
	...
	index("idx_watch_party_participants_user").using("btree", table.userId.asc().nullsLast().op("uuid_ops")),
	foreignKey({
			columns: [table.sessionId],
			foreignColumns: [watchPartySessions.id],
			name: "watch_party_participants_session_id_fkey"
		}).onDelete("cascade"),
	foreignKey({
			columns: [table.userId],
			foreignColumns: [profiles.id],
			name: "watch_party_participants_user_id_fkey"
		}),
	primaryKey({ columns: [table.sessionId, table.userId], name: "watch_party_participants_pkey"}),
]);
